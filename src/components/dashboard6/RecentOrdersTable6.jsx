...
import { Search, Filter, MoreHorizontal, ChevronLeft, ChevronRight, ClipboardList } from "lucide-react";
...
export default function RecentOrdersTable6() {
              ...
              {paged.length === 0 ? (
                ...
                paged.map((order, i) => (
                  <tr key={order.id} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3.5">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium border ${statusStyles[order.status]}`}>
                        {order.status}
                      </span>
                    </td>
                    <td className="px-4 py-3.5 font-medium tabular-nums whitespace-nowrap">{order.total}</td>
