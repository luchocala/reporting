...
export default function D5CategoryPerformance() {
          ...
          <div className="flex items-center border rounded-md overflow-hidden shrink-0">
            {["week", "month"].map((v) => (
              <button key={v} onClick={() => setView(v)}
                className={`px-3 py-1.5 text-xs font-medium capitalize transition-colors ${view === v ? "bg-foreground text-background" : "text-muted-foreground hover:bg-muted"}`}>
                {v === "week" ? "Week" : "Month"}
              </button>
            ))}
          </div>
        ...
        <div className="h-52">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              {LINES.map((l) => <Line key={l.key} type="monotone" dataKey={l.key} stroke={l.color} strokeWidth={1.5} dot={false} name={l.label} />)}
            </LineChart>
          </ResponsiveContainer>
        </div>
      </CardContent>
