...
import {
  ...
  ShieldCheck, AlertTriangle, CheckSquare, Check, Plus,
} from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
    ...
    items: [
      ...
      },
      {
        icon: Package, label: "Products", children: [
          ...
          { label: "Edit Product", path: "/ecommerce/edit-product/radiance-ritual-set" },
          { label: "Edit Product 2", path: "/ecommerce/edit-product-2/radiance-ritual-set" },
      ...
      {
        icon: ShoppingCart, label: "Orders", children: [
      ...
      <li>
        <button
          ...
          className={`flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-sm transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground ${
            isActive ? "font-medium text-sidebar-foreground" : "text-sidebar-foreground/60"
          }`}
        >
          ...
          <DropdownMenuTrigger asChild>
            <button className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 hover:bg-sidebar-accent transition-colors text-left">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold leading-tight truncate text-sidebar-foreground">{activeTeam.name}</p>
                <p className="text-[11px] text-sidebar-foreground/50 leading-tight truncate">{activeTeam.plan}</p>
              </div>
          ...
          <DropdownMenuContent className="w-52" align="start" side="bottom">
            ...
            {teams.map((team) => (
              ...
              >
                ...
                {activeTeam.name === team.name && <Check className="size-3.5 text-primary ml-auto" />}
              </DropdownMenuItem>
            ))}
            <DropdownMenuSeparator />
